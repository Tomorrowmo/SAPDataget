...
# 喂给 LLM 的样本行数(省 token + 避免敏感数据大面积外泄)。导出 Excel 是独立的全量路径。
_LLM_SAMPLE_ROWS = 5
# 样本查询向 BW 要的最大行数(只为算样本+总数,不必拉全量)。
_SAMPLE_QUERY_TOP = 50
...
- execute_odata_query  : { "service","entity_set","where",("select"),("orderby"),("top"),("apply") } —— 跑查询(只回少量样本+总数)
- build_excel       : { "service","entity_set","where",("select"),("orderby"),("top"),("apply"),("sheet_title") } —— 收尾出全量 Excel
...
6. **过滤条件用结构化 `where`,不要自己拼 $filter 字符串**(后端会按字段类型安全转义,防注入/防撇号崩溃):
   - where 是条件数组,每条 `{ "field": <字段>, "op": <运算符>, "value": <值> }`,多条之间是 AND。
   - op ∈ eq | ne | gt | ge | lt | le | contains | startswith | endswith。
   - 字段名**必须**来自 get_service_metadata 的真实字段,不要臆造。
   - 值按字段类型给:字符串/ID 直接给原值(后端自动加引号转义,**你不要自己加引号**);
     数值直接给数字;日期字段(Edm.DateTime)给 'YYYY-MM-DD'。
   - 注意:本系统 `CALMONTH` 是**字符串**类型,值形如 `"202605"`(YYYYMM),不是日期。
   - 例:查 2026年5月华东 → where: [{"field":"CALMONTH","op":"eq","value":"202605"},{"field":"Region","op":"eq","value":"HD"}]
   - 例:客户名含 "上海" → where: [{"field":"CustomerName","op":"contains","value":"上海"}]
   - 默认 top=100;用户要"全部"再调大(后端会自动分页拉全量,你不用关心分页)。
...
class StreamAgent:
    ...
    def __init__(
        ...
        *,
        ...
        sensitive_resolver: Callable[[str], dict[str, str]] | None = None,
    ) -> None:
        """on_excel: 产出 Excel 时的回调 (server 注入) —— 负责把文件登记进 DB 并返回
        前端可用的 task payload (含 download_url)。CLI 不传则用本地路径兜底。

        sensitive_resolver: (service) -> {field: mask_mode}。用于在**把样本行喂给 LLM 之前**
        脱敏(P1-3)。不传则不脱敏(CLI / 无敏感配置)。Excel 侧脱敏由 orchestrator 负责。"""
        ...
        self.sensitive_resolver = sensitive_resolver
        self.total_input_tokens = 0
        self.total_output_tokens = 0
        self.last_task: TaskResult | None = None
        self._meta_cache: dict[str, dict[str, Any]] = {}      # service -> 简化 metadata

    # ---------- OData 辅助 ----------
    def _types_for(self, service: str, entity_set: str) -> dict[str, str]:
        """取某 EntitySet 的 {字段: Edm 类型}(metadata 进程内缓存)。"""
        if service not in self._meta_cache:
            resp = self.bw.get_metadata(service)
            self._meta_cache[service] = resp.json if (not resp.error and isinstance(resp.json, dict)) else {}
        return odata.prop_types(self._meta_cache[service], entity_set)

    def _resolve_filter(self, args: dict[str, Any], service: str, entity_set: str) -> str | None:
        """优先用结构化 where(安全拼装,防注入);否则退回裸 filter 串(兼容)。"""
        where = args.get("where")
        if isinstance(where, list) and where:
            types = self._types_for(service, entity_set)
            return odata.build_filter(where, types) or None
        f = args.get("filter")
        return f or None

    def _mask_sample(self, service: str, rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
        """喂 LLM 前对样本行脱敏(P1-3),复用 Excel 同一套脱敏函数,口径一致。"""
        if not self.sensitive_resolver or not rows:
            return rows
        try:
            smap = self.sensitive_resolver(service) or {}
        except Exception:                                     # noqa: BLE001
            return rows
        if not smap:
            return rows
        cols = list(rows[0].keys())
        masked, _ = apply_sensitive_mask(rows, cols, smap)
        return masked
    ...
    def _dispatch(
        ...
        try:
            ...
            if action == "execute_odata_query":
                # 样本路径:只向 BW 要少量行(_SAMPLE_QUERY_TOP)用于验证+总数,喂 LLM 前脱敏。
                try:
                    flt = self._resolve_filter(args, args["service"], args["entity_set"])
                except ValueError as e:
                    return {"status": "error", "error": f"过滤条件非法: {e}"}, None
                sample_top = min(int(args.get("top", _SAMPLE_QUERY_TOP) or _SAMPLE_QUERY_TOP), _SAMPLE_QUERY_TOP)
                resp = self.bw.execute_query(
                    service=args["service"], entity_set=args["entity_set"],
                    filter=flt, select=args.get("select"),
                    orderby=args.get("orderby"), top=sample_top,
            ...
            if action == "build_excel":
                # 导出路径:全量(由 live 分页 + max_export_rows 兜底)。
                try:
                    flt = self._resolve_filter(args, args["service"], args["entity_set"])
                except ValueError as e:
                    return {"status": "error", "error": f"过滤条件非法: {e}"}, None
